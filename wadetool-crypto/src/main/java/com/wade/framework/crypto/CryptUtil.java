...
public class CryptUtil {
    ...
    // 48
    static final int pc_2p[] = {14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13, 2, 41, 52, 31, 37, 47, 55, 30,
    ...
    // 64
    static final int iip_tab_p[] = {58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46, 38, 30, 22, 14, 6, 64, 56, 48, 40, 32,
            24, 16, 8, 57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3, 61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7};
    
    // 64
    static final int _iip_tab_p[] = {40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53,
            21, 61, 29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25};
    
    // 48
    static final int e_r_p[] = {32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20, 21, 22,
            23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1};
    
    // 32
    static final int local_PP[] = {16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4,
            25};
    ...
    private byte[] packKeyTo8Byte(byte[] str) {
        byte[] skey = new byte[8];
        
        skey[0] = (byte)(str[0] >> 1);
        skey[1] = (byte)(((str[0] & 0x01) << 6) | (str[1] >> 2));
        skey[2] = (byte)(((str[1] & 0x03) << 5) | (str[2] >> 3));
        skey[3] = (byte)(((str[2] & 0x07) << 4) | (str[3] >> 4));
        skey[4] = (byte)(((str[3] & 0x0F) << 3) | (str[4] >> 5));
        skey[5] = (byte)(((str[4] & 0x1F) << 2) | (str[5] >> 6));
        skey[6] = (byte)(((str[5] & 0x3F) << 1) | (str[6] >> 7));
        skey[7] = (byte)((str[6] & 0x7F) | str[7]);
        for (int i = 0; i < 8; i++) {
    ...
    private byte[] packKeyTo8Byte(String key) {
        if (key.length() == 0)
            return packKeyTo8Byte("12345678".getBytes());
        else if (key.getBytes().length == 8) {
            return packKeyTo8Byte(key.getBytes());
        }
        else if (key.getBytes().length < 8) {
            byte[] b = new byte[8];
            for (int i = 0; i < 8; i++) {
            ...
            return packKeyTo8Byte(b);
        }
        else {
            String skey = "";
            if (key.getBytes().length % 8 != 0) {
                skey = key + "        ";
            }
            else {
                skey = key;
            }
            byte[] bori = skey.getBytes();
            byte[] b = new byte[8];
            for (int i = 0; i < 8; i++) {
                b[i] = 0;
            }
            for (int i = 0; i < bori.length / 8; i++) {
                for (int j = 0; j < 8; j++) {
                    b[j] = (byte)((bori[i * 8 + j] ^ b[j]) & 0xFF);
                }
            }
            return packKeyTo8Byte(b);
        }
    ...
    public String inner_encrypt(String key, String oriStr) {
        byte[] mkey = packKeyTo8Byte(key);
        byte[] orib = oriStr.getBytes();
        byte[] enb = new byte[4096];
        int i1 = ENCRYPT(mkey, orib, enb, orib.length);
        String digestHexStr = "";
        for (int i = 0; i < i1; i++) {
            digestHexStr += byteHEX(enb[i]);
        }
        return digestHexStr;
    }
     ...
     * @param oriStr
     * @return
     * @Date        2017年11月29日 下午2:35:50 
     * @Author      yz.teng
     */
    public static String encrypt(String key, String oriStr) {
        CryptUtil ct = new CryptUtil();
        return ct.inner_encrypt(key, oriStr);
    }
    
    public String inner_decrypt(String key, String enStr) {
        byte[] mkey = packKeyTo8Byte(key);
        byte[] enByte = hexStr2ByteArr(enStr);
        byte[] oriByte = new byte[4096];
        int i2 = DECRYPT(mkey, oriByte, enByte, enByte.length);
        String s = new String(oriByte, 0, i2);
        return new String(oriByte, 0, i2).trim();
    }
     ...
     * @param enStr
     * @return
     * @Date        2017年11月29日 下午2:36:02 
     * @Author      yz.teng
     */
    public static String decrypt(String key, String enStr) {
        CryptUtil ct = new CryptUtil();
        return ct.inner_decrypt(key, enStr);
    }
    
    public static void main(String[] args) throws ParseException {
        String enkey = "aaaaaaaaaaabbbbbbbbbbbbccccccccccc";
        String before = System.nanoTime() + "";
        System.out.println("加密前=:" + before);
        String encrypt = CryptUtil.encrypt(enkey, before);
        System.out.println("加密后=:" + encrypt);
        System.out.println("解密=:" + CryptUtil.decrypt(enkey, encrypt));
        System.out.println("解密=:" + CryptUtil.decrypt("123", encrypt));
