...
/**
 * redis工具类(用于业务处理缓存)
 * @Description redis工具类(用于业务处理缓存)
 * @ClassName   RedisFactory 
 * @Date        2018年4月19日 上午1:18:15 
 * @Author      yz.teng
 */
public class RedisFactoryBusinessSlave1 {
    private static final Logger log = LogManager.getLogger(RedisFactoryBusinessSlave1.class);
                    ...
                    try {
                        if (jedisPool == null) {
                            ...
                            if (password != null && zkProxyAuth != null) {
                                ...
                                log.info("zkProxyAuth=:" + zkProxyAuth);
    ...
    private final static Properties getProviderProperties(Properties props) {
        ...
        String prefix = "redis.business.slave1.";
    ...
    public static Long getTTL(String key) {
        ...
        try {
            ...
            log.error("RedisFactory redis异常,getTTL error:", e);
        }
        finally {
            close(jedis);
        }
