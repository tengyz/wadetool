...
public class ParamTablesCache extends BaseReadOnlyCache {
    private static final transient Logger log = Logger.getLogger(ParamTablesCache.class);
    ...
    public Map<String, Object> loadData() throws Exception {
        ...
        if (FROM_DB) {
            //获取缓存配置表WT_PARAM_TABLES
            IDataList getList = (IDataList)loadTableData("RP_PARAM_TABLES");
            if (log.isDebugEnabled()) {
                log.debug("===查询数据库=ParamTablesCache=loadData()==getList=:" + getList);
            }
        ...
        log.debug("========ParamTablesCache===loadData()=====map=:" + map);
        return map;
    }
